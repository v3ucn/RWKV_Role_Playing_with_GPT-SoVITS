...
class UI:
  ...
  def __send_message(self, message, top_p, top_k, temperature, presence_penalty, context_penalty, replace_message,if_read,api_url):
    ...
    print(self.chat_model.r_text)

    if if_read:

      read_now(self.chat_model.r_text,api_url)
  ...
  def create_ui(self):
    with gr.Blocks(title=self.language_conf['TITLE']) as app:
        ...
        with gr.Row():
          ...
          with gr.Column(scale=1):
            with gr.Tab(self.language_conf['TAB_DEFAULT']):
              ...
              with gr.Row():
                with gr.Column(min_width=100):
                  if_read = gr.Checkbox(label="是否开启GPT-SoVITS语音朗读",value=False)
              with gr.Row():
                with gr.Column(min_width=100):
      ...
      message.submit(self.__send_message, inputs=input_list + [replace_message]+ [if_read,api_url], outputs=output_list + interactive_list + [replace_message]).then(self.__arrange_token, outputs=interactive_list, show_progress=False)
      submit.click(self.__send_message, inputs=input_list + [replace_message] + [if_read,api_url], outputs=output_list + interactive_list + [replace_message]).then(self.__arrange_token, outputs=interactive_list, show_progress=False)
